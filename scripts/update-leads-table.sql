...
-- Add check constraints for product categories
DO $$
BEGIN
  -- Add check constraint for product_category if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM information_schema.check_constraints WHERE constraint_name='leads_product_category_check') THEN
    ALTER TABLE leads ADD CONSTRAINT leads_product_category_check 
    CHECK (product_category IN ('Motor', 'Health', 'Life', 'Travel', 'Pet', 'Cyber', 'Corporate', 'Marine') OR product_category IS NULL);
  END IF;
END $$;
...
SET 
  ...
  product_subtype = 'International'
WHERE product_category IS NULL AND id = 1;
